	...
	public class ProfilePageView : ContentPage
	{
		...
		Frame createPhotoBlock()
		{
			...
			var roundedLayout = new Frame
			{
				...
				Padding = 0,
				HasShadow = false,
				BackgroundColor = Color.FromArgb(Theme.Current.SettingsTableColor),
				Content = new StackLayout
				{
					Children =
					{
		...
		Image createSectionIcon(string icon)
		{
			return new Image
			{
		...
		Label createLabel()
		{
			return new Label
			{
				TextColor = Color.FromArgb(Theme.Current.SettingsProfileLabelColor),
		...
		View createAvatar()
		{
			...
			avatarImage.Triggers.Add(new DataTrigger(typeof(Image))
			{
				Binding = new Binding("Avatar"),
				Value = null,
				Setters = { new Setter { Property = Image.IsVisibleProperty, Value = false } }
			});
