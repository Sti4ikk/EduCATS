...
namespace EduCATS.Pages.Today.Base.ViewModels
{
	public class TodayPageViewModel : ViewModel
	{
		...
		private Command<int> _calendarPositionChangedCommand;

		public Command<int> PositionSelectedCommandProperty
		{
			get
			{
				return _calendarPositionChangedCommand ??= new Command<int>(calendarPositionChangedEvent);
			}
		}
		...
		async Task getUpdateMessage()
		{
				 ...
				 Convert.ToInt32(a[1]) == Convert.ToInt32(b[1])))
			{
				...
				var result = await _services.Dialogs.ShowMessageUpdate(
					title,
					message + version,
					linkButton,
					cancelButton);
		...
		protected void calendarPositionChangedEvent(int position)
		{
			try
			{
